...
//PIERDE RASTRO DE LA TERCER COLUMNA
//const thirdColumn = document.querySelector('#thrid-column') // Obtenemos el titulo de la 3 columna, que debe cambiar según lo que muestre.
...
const btnDate = () =>{
    document.querySelector('#thrid-column').innerHTML = '<th>DNI</th>'
...
const btnPromedios = () => {
    deleteColumn()
    document.querySelector('#thrid-column').innerHTML = '<th>Promedio</th>'
    bodyTableAlumnos.innerHTML = ''
    alumnos.forEach(alumno => {
        bodyTableAlumnos.innerHTML += `<tr>
        <td>${alumno.nombre}</td>
        <td>${alumno.apellido}</td>
        <td>${parseInt((alumno.examen1 + alumno.examen2 + alumno.examen3)/3)}</td>
    </tr>`
...
const btnAsistencia = () => {
    deleteColumn()
    bodyTableAlumnos.innerHTML = ''
    headerTable.innerHTML += "<th id=\"delete\">Promedios</th>"
    document.querySelector('#thrid-column').innerHTML = '<th>Asistencia</th>'
    alumnos.forEach(alumno => {
        bodyTableAlumnos.innerHTML += `<tr>
        <td>${alumno.nombre}</td>
        <td>${alumno.apellido}</td>
        <td>${alumno.asistencias}/24</td>
        <td id="delete">${parseInt(alumno.asistencias/24*100)}%</td>
        </tr>`
    })
    addColumn = true
} 
...
const btnAprobados = () => {
    deleteColumn()
    document.querySelector('#thrid-column').innerHTML = 'Promedio'
    bodyTableAlumnos.innerHTML = ''
    alumnos.forEach(alumno => {
        let promedio = parseInt((alumno.examen1 + alumno.examen2 + alumno.examen3)/3)
        if (promedio >= 70) {
        bodyTableAlumnos.innerHTML += `<tr>
        <td>${alumno.nombre}</td>
        <td>${alumno.apellido}</td>
